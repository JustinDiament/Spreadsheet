...
describe('Sum Strategy', (): void => {   
    ...
    let strategyWithSetValues: SumStrategy = new SumStrategy(otherCells, 100, 100)
    let strategyWithDefaultValues: SumStrategy = new SumStrategy(otherCells, 100, 100)

    /// TODO Add tests for errors like self ref
  
    it('should take return the value of the cell if only one defined', (): void => {
        expect(strategyWithSetValues.parse("SUM(A1..A1)")).toBe("0");
    });

    it('should return the sum of a row of cells with number values', (): void => {
        expect(strategyWithSetValues.parse("SUM(A1..E1)")).toBe("2");
    });

    it('should return the sum of a selection of cells with number values', (): void => {
        expect(strategyWithSetValues.parse("SUM(A1..E2)")).toBe("20");
    });

    it('should return an error if string values are involved', (): void => {
        expect(strategyWithSetValues.parse("SUM(A1..A3)")).toBe("ERROR: Connot take sum of non-numbers");
    });

    it('should return an error if there are no values', (): void => {
        expect(strategyWithDefaultValues.parse("SUM(A1..A3)")).toBe("ERROR: Connot take sum of non-numbers");
    });
