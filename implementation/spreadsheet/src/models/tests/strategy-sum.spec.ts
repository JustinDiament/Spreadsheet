...
// Tests for the sum range expression strategy
describe("Sum Strategy", (): void => {
  ...
  // Test that a single cell's sum can be calculated
  it("should sum a single cell", (): void => {
    ...
    let strategy = new SumStrategy(cells, 0, 2);
    expect(strategy.parse("SUM(A1..A1)")).toBe("5");
  });

  // Test that a group of cells' sum can be calculated
  it("should sum a group of cells", (): void => {
    // Set up cells with numbers
    cell.setEnteredValue("5");
    ...
    // Set up a SumStrategy for another cell
    let strategy = new SumStrategy(cells, 0, 2);
    expect(strategy.parse("SUM(A1..B2)")).toBe("50");
  });
  ...
  it("should maintain other text in dispaly value", (): void => {
    ...
    // Check that the sum expressions are replaced with the display value 
    expect(strategy.parse("hello SUM(A1..B1) bye")).toBe("hello 15 bye");
  });

  // Test that there can be other things in the cell text and they will remain intact
  it("should allow for multiple summations", (): void => {
    // Set up a cell with a string
    cell.setEnteredValue("5");
    cell.updateDisplayValue(cells);
    cell2.setEnteredValue("10");
    ...
    // Check that the sum expressions are replaced with the display value 
    expect(strategy.parse("hello SUM(A1..B1) bye SUM(A1..A1)")).toBe("hello 15 bye 5");
  });
});
