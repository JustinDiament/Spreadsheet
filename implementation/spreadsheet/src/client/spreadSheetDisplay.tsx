...
export default function SpreadSheetDisplay() {
  ...
  const findReplaceOpenF: () => boolean = () =>
    "find and replace" === currPanel && sidePanel;
  ...
  const menuFunctions: Array<{ (): void } | { (val: string): void }> = [
    ...
    useSpreadsheetController(
      (controller: ISpreadSheetState) => controller.deleteRow
    ),
    // add row above
    useSpreadsheetController(
      (controller: ISpreadSheetState) => controller.addRow
    ),
    // add row below
    useSpreadsheetController(
      (controller: ISpreadSheetState) => controller.addRow
    ...
    // delete column(s)
    useSpreadsheetController(
      (controller: ISpreadSheetState) => controller.deleteColumn
    ),
    // add column above
    useSpreadsheetController(
      (controller: ISpreadSheetState) => controller.addColumn
    ),
    // add column below
    useSpreadsheetController(
      (controller: ISpreadSheetState) => controller.addColumn
    ...
    // clear selected cells
    useSpreadsheetController(
      (controller: ISpreadSheetState) => controller.clearSelectedCells
    ),
    // clear all cells
    useSpreadsheetController(
      (controller: ISpreadSheetState) => controller.clearAllCells
    ),
  ];
    ...
    <div>
      ...
      <div
        ...
        <div className="position-fixed top-50 start-50 translate-middle bg-white sp-help-menu ">
          ...
          <div
            className={"sp-panel-close float-right m-0 p-0"}
      ...
      <div className="d-flex flex-nowrap sp-menu-bar w-100 align-items-center">
        ...
        <div
          ...
          <button
            className={
              "sp-menu-button " + (dropDisplayState("edit") ? "selected" : "")
            }
            type="button"
            aria-haspopup="menu"
        ...
        <div
          ...
          <button
            className={
              "sp-menu-button " + (dropDisplayState("data") ? "selected" : "")
            }
            type="button"
            aria-haspopup="menu"
            aria-expanded={dropdown ? "true" : "false"}
      ...
      <div className={(sidePanel ? "sp-two-panel" : "") + " sp-work-space"}>
        ...
        <CellGridDisplay
          findReplaceOpen={() => "find and replace" === currPanel && sidePanel}
        />
        ...
        <div
          ...
          <div
            className={"sp-panel-close float-right"}
          ...
          <FindReplaceMenu disp={findReplaceOpenF()} />
        </div>
      </div>
    </div>
  );
